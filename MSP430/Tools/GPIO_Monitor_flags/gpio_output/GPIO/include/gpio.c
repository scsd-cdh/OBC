...
uint8_t  GPIO_getOutput(uint8_t selectedPort,uint16_t selectedPins){
    // Get the Base Address of Chosen Port
    uint16_t baseAddress = GPIO_PORT_TO_BASE[selectedPort];
    ...
    // Shift by 8-bit if port id even (upper 8-bit)
    if((selectedPort & 1) ^ 1) {
        selectedPins <<= 8;
    }

    return HWREG16(baseAddress + OFS_PAOUT);
}
